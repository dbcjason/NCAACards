...
def load_enriched_lookup(
    ...
    for season in sorted(seasons):
        ...
        for p in players:
            ...
            by_key[(season, norm(key), norm(team))] = rec
            name_key = (season, norm(key))
...
def build_rows(root: Path, bt_csv: Path, gender: str, seasons: set[int], min_games: float, min_min_per: float, men_rimfluence_cutoff: int) -> list[Row]:
    ...
    enr_by_key, enr_by_name = load_enriched_lookup(root, gender, seasons)
    ...
    for r in bt_rows:
        ...
        em = enr_by_key.get(k, {})
        if not em:
            em = enr_by_name.get((season, norm(player)), {})
