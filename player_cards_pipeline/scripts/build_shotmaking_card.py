...
def has_min_enriched_possessions(row: dict[str, Any], min_poss: float = 1000.0) -> bool:
    v = ((row.get("off_team_poss") or {}).get("value")) if isinstance(row.get("off_team_poss"), dict) else None
    try:
        poss = float(v if v is not None else 0.0)
    except Exception:
        poss = 0.0
    return math.isfinite(poss) and poss >= min_poss
...
def build(args: argparse.Namespace) -> str:
    ...
    same = [
        ...
        and has_min_enriched_possessions(p, 1000.0)
    ]
