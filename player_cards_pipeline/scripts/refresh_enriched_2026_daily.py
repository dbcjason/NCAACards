...
This downloads a zip that contains `enrichedPlayers/players_all_Men_2025_*.json`,
...
DEFAULT_DROPBOX_URL = (
    ...
    "hoop_explorer_players_all_2025_26.zip?dl=0&e=2&file_subpath=%2FenrichedPlayers&"
    "rlkey=xv5e4n1rx7n109mschdsfpqlo"
)
...
def find_enriched_jsons(extract_root: Path) -> list[Path]:
    # Keep only json year 2025 files, since script season 2026 expects that mapping.
    found = sorted(extract_root.rglob("players_all_Men_2025_*.json"))
    return [p for p in found if p.name.endswith(".json")]


def copy_sources(src_files: list[Path], dst_dir: Path) -> None:
    dst_dir.mkdir(parents=True, exist_ok=True)
    for p in dst_dir.glob("players_all_Men_2025_*.json"):
        p.unlink()
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="enriched_zip_") as td:
        ...
        src_files = find_enriched_jsons(extract_dir)
        if not src_files:
    ...
    expected = by_script / "players_all_Men_scriptSeason_2026_fromJsonYear_2025.json"
