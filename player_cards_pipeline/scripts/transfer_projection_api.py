...
def auth_token() -> str:
    return string_or_empty(os.getenv("TRANSFER_MODEL_TOKEN")) or string_or_empty(os.getenv("PAYLOAD_SYNC_TOKEN"))


def string_or_empty(value: str | None) -> str:
...
class TransferProjectionHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            player = string_or_empty(body.get("player"))
            team = string_or_empty(body.get("team"))
            season = string_or_empty(body.get("season"))
            destination_conference = string_or_empty(body.get("destinationConference"))
