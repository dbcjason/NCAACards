...
# Big-only profile (no enriched skills, only size-dependent box/impact indicators).
...
def load_enriched_map(enriched_dir: Path, seasons: set[int]) -> dict[tuple[int, str], dict[str, float]]:
    ...
    for season in sorted(seasons):
        matches = sorted(enriched_dir.glob(f"players_all_Men_scriptSeason_{season}_fromJsonYear_*.json"))
        if not matches:
            continue
        path = matches[0]
        try:
...
def main() -> None:
    ...
    ap.add_argument("--out-model-json", default="player_cards_pipeline/data/models/height_profile_model_v1.json")
