...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help="Directory containing players_all_Men_<year>_<tier>.json files.",
    )
    ap.add_argument(
        "--output-dir",
...
def file_meta(path: Path) -> tuple[str, str] | None:
    # players_all_Men_2024_High.json
    name = path.name
...
def main() -> None:
    ...
    for p in sorted(in_dir.glob("players_all_Men_*_*.json")):
        meta = file_meta(p)
        if meta is None:
            continue
        year, tier = meta
    ...
    for year in sorted(files_by_year.keys()):
        ...
        out_json_year = by_json_year_dir / f"players_all_Men_{year}_combined.json"
        with out_json_year.open("w", encoding="utf-8") as f:
            json.dump(out_obj, f, ensure_ascii=False)

        out_script_season = (
            by_script_season_dir
            / f"players_all_Men_scriptSeason_{int(year)+1}_fromJsonYear_{year}.json"
        )
