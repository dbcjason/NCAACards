...
_TRANSFER_PROJECTION_CACHE: dict[int, dict[str, Any]] = {}
...
def norm_team(v: Any) -> str:
    ...
    aliases = {
        ...
        # School-name variants between BT and enriched dumps.
        "mcneese": "mcneese",
        "mcneesest": "mcneese",
        "mcneesestate": "mcneese",
        "centralark": "centralark",
        "centralarkansas": "centralark",
        "centralconnecticut": "centralconn",
        "centralconnst": "centralconn",
        "centralconnecticutst": "centralconn",
        "ccsu": "centralconn",
        "loyolamd": "loyolamaryland",
        "loyolamaryland": "loyolamaryland",
        "selouisiana": "southeasternla",
        "southeasternlouisiana": "southeasternla",
        "southeasternla": "southeasternla",
        # St. Thomas appears in multiple variants across sources.
        "stthomas": "stthomas",
        "stthomasmn": "stthomas",
        "stthomasminn": "stthomas",
        "stthomasminnesota": "stthomas",
        "saintthomasmn": "stthomas",
        "saintthomasminn": "stthomas",
        "saintthomasminnesota": "stthomas",
...
def norm_player_name(v: Any) -> str:
    s = str(v or "").strip()
    if "," in s:
        # Handle both "Last, First" and "Last, Suffix, First" formats from enriched feeds.
        ...
        if len(parts) >= 3:
            ...
            first = parts[-1]
            middles = parts[1:-1]
            s = " ".join([first, last] + middles).strip()
        elif len(parts) == 2:
            left, right = parts
            # If right side is only a suffix token, keep left as "First Last".
            right_key = re.sub(r"[^a-z0-9]+", "", right.lower())
            if right_key in suffixes:
...
def find_enriched_row(
    ...
    # Fallback: if a player-season is unique in enriched, use it regardless of team label variance.
...
def inject_enriched_fields_into_bt_rows(
    ...
    for r in bt_rows:
        ...
        er = find_enriched_row(lookup, player_name, team_name, ys)
        if not er:
            continue
...
def _build_transfer_projection_cache(bt_rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    by_dest_conf: dict[str, list[dict[str, Any]]] = defaultdict(list)
    impact_keys = ["bpm", "rapm", "net_pts"]
    impact_scores_by_dest: dict[str, list[float]] = defaultdict(list)
    overall_impact_scores: list[float] = []
    feat_keys = [
        "mpg", "ppg", "rpg", "apg", "spg", "bpg", "fg_pct", "tp_pct", "ft_pct",
        "bpm", "usg", "ts_per", "rim_pct", "ast_per", "ast_tov",
        "stl_per", "blk_per", "orb_per", "drb_per", "rapm", "net_pts", "onoff_net_rating",
    ]
    source_value_buckets: dict[str, list[float]] = defaultdict(list)
    source_value_buckets_by_dest: dict[str, dict[str, list[float]]] = defaultdict(lambda: defaultdict(list))
    ...
    for _p, season_map in by_player_year.items():
        ...
        for y in years:
            ...
            by_dest_conf[dst_conf].append(example)
            for k in feat_keys:
                v = src_m.get(k)
                if v is None or not math.isfinite(float(v)):
                    continue
                source_value_buckets[k].append(float(v))
                source_value_buckets_by_dest[dst_conf][k].append(float(v))
            vals = [dst_m[k] for k in impact_keys if k in dst_m and math.isfinite(float(dst_m[k]))]
            if vals:
                score = sum(float(v) for v in vals) / len(vals)
                overall_impact_scores.append(score)
                impact_scores_by_dest[dst_conf].append(score)

    def build_scales(values_by_key: dict[str, list[float]]) -> dict[str, float]:
        scales: dict[str, float] = {}
        for k in feat_keys:
            vals = sorted(values_by_key.get(k, []))
            if len(vals) >= 12:
    ...
    scales_by_dest = {conf: build_scales(values_by_key) for conf, values_by_key in source_value_buckets_by_dest.items()}
    overall_scales = build_scales(source_value_buckets)
    return {
        "examples": examples,
        "by_dest_conf": dict(by_dest_conf),
        "impact_scores_by_dest": dict(impact_scores_by_dest),
        "overall_impact_scores": overall_impact_scores,
        "scales_by_dest": scales_by_dest,
        "overall_scales": overall_scales,
    }


def _get_transfer_projection_cache(bt_rows: list[dict[str, str]]) -> dict[str, Any]:
    key = id(bt_rows)
    cached = _TRANSFER_PROJECTION_CACHE.get(key)
    if cached is None:
        cached = _build_transfer_projection_cache(bt_rows)
        _TRANSFER_PROJECTION_CACHE[key] = cached
    return cached
...
def build_transfer_projection_html(target: PlayerGameStats, destination_conference: str, bt_rows: list[dict[str, str]]) -> str:
    ...
    projection_cache = _get_transfer_projection_cache(bt_rows)
    examples: list[dict[str, Any]] = projection_cache["examples"]
...
"""
    ...
    if isinstance(cached_row, dict):
        cached_projection = (cached_row.get("projections") or {}).get(dest_conf) if isinstance(cached_row.get("projections"), dict) else {}
    ...
    same_dest = list(projection_cache["by_dest_conf"].get(dest_conf, []))
    ...
    scales = (
        projection_cache["scales_by_dest"].get(dest_conf, projection_cache["overall_scales"])
        if len(same_dest) >= 35
        else projection_cache["overall_scales"]
    )
...
"""
    ...
    # Transfer grade impact score excludes On/Off NetR.
    impact_keys = ["bpm", "rapm", "net_pts"]
    ...
    if pred_impact:
        ...
        cohort_scores = (
            projection_cache["impact_scores_by_dest"].get(dest_conf, projection_cache["overall_impact_scores"])
            if len(same_dest) >= 20
            else projection_cache["overall_impact_scores"]
        )
        if cohort_scores:
...
def _build_draft_projection_context(
    bt_rows: list[dict[str, str]],
    bio_lookup: dict[tuple[str, str, str], dict[str, str]],
    ...
    row_feature_vecs: dict[int, dict[str, float]] = {}
    age_height_by_row: dict[int, tuple[float | None, float | None]] = {}
    rsci_score_by_row: dict[int, float | None] = {}
    for r in rows_for_maps:
        y = norm_season(bt_get(r, ["year"]))
        age, hgt = row_age_height(r)
        age_height_by_row[id(r)] = (age, hgt)
        row_rsci = row_rsci_score(r)
        rsci_score_by_row[id(r)] = row_rsci
        vec: dict[str, float] = {}
        for key in metric_keys:
            p = metric_pct_map.get((y, key), {}).get(id(r))
            if p is not None and math.isfinite(p):
                vec[key] = float(p)
        row_feature_vecs[id(r)] = vec
    ...
    for r in bt_rows:
        ...
        age, hgt = age_height_by_row.get(id(r), (None, None))
        row_rsci = rsci_score_by_row.get(id(r))
        vec = row_feature_vecs.get(id(r), {})
        comps = feature_components(vec, age, hgt, row_rsci)
        if len(comps) < 6:
            continue
    ...
    return {
        ...
        "score_from_components": score_from_components,
        "feature_components": feature_components,
        "row_feature_vecs": row_feature_vecs,
        "age_height_by_row": age_height_by_row,
        "rsci_score_by_row": rsci_score_by_row,
    }
...
def build_draft_projection_html(
    ...
    target_year = int(ys)
    cache_key = (id(bt_rows), id(bio_lookup), id(rsci_map), target_year)
    ctx = _DRAFT_PROJECTION_CONTEXT_CACHE.get(cache_key)
    if ctx is None:
        ctx = _build_draft_projection_context(bt_rows, bio_lookup, rsci_map, target_year)
        _DRAFT_PROJECTION_CONTEXT_CACHE[cache_key] = ctx

    metric_keys = ctx["metric_keys"]
    feature_components = ctx["feature_components"]
    score_from_components = ctx["score_from_components"]

    target_vec = dict(ctx["row_feature_vecs"].get(id(target_row), {}))
    t_age, t_hgt = ctx["age_height_by_row"].get(id(target_row), (None, None))
    t_rsci = ctx["rsci_score_by_row"].get(id(target_row))
    if t_rsci is None:
    ...
    candidates_raw: list[dict[str, Any]] = ctx["candidates_raw"]
    ...
    learned_weights: dict[str, float] = ctx["learned_weights"]
    ...
    return f"""
      <div class="panel draft-proj-panel">
        ...
        <div class="draft-proj-sub draft-proj-note">Projections based solely on statistical profile in an average draft</div>
      </div>
"""
...
def build_player_comparisons_html(
    ...
    def similarity(other: dict[str, str]) -> float | None:
        ...
        if enforce_age_window and target_age_raw is not None and math.isfinite(target_age_raw):
        ...
        for p in candidates:
            ...
            with p.open("r", encoding="utf-8-sig", newline="") as f:
                for row in csv.DictReader(f):
                    row_season = norm_season(row.get("season", ""))
                    if row_season and row_season != s:
                        continue
                    delta = to_float(row.get("big_height_delta_inches"))
                    if delta is None:
                        delta = to_float(row.get("height_delta_inches"))
                    if delta is None or not math.isfinite(delta):
                        continue
                    name = norm_player_name(row.get("player_name", ""))
...
def compute_statistical_height_delta(
    ...
    row = target_row if target_row is not None else (bt_find_target_row(bt_rows, target) if bt_rows else None)
    if not row:
        return None
    ...
    for name in feature_names:
        if name in bt_aliases:
            x.append(bt_num(row, bt_aliases[name]) or 0.0)
        elif name in enriched_map:
            x.append(bt_num(row, enriched_map[name]) or 0.0)
        else:
            x.append(0.0)

    z_values: list[float] = []
    for j in range(n):
        sd = _num_or(std_x[j], 1.0)
        if abs(sd) < 1e-9:
            sd = 1.0
        z_values.append((x[j] - _num_or(mean_x[j], 0.0)) / sd)

    if is_big_v2:
        score = sum(_num_or(positive_corr_weights[j], 0.0) * z_values[j] for j in range(n))
        pred = (_num_or(linear_map.get("a"), 0.0) * score) + _num_or(linear_map.get("b"), 0.0)
    else:
        pred = bias + sum(_num_or(weights[j], 0.0) * z_values[j] for j in range(n))
...
def build_per_game_percentiles(
    ...
    ys = norm_text(target.season)
    target_bucket = ""
    if bt_rows:
        bt_index = _get_bt_row_index(bt_rows)
        target_row = bt_index["exact"].get(
            (norm_player_name(target.player), norm_team(target.team), norm_season(target.season))
        )
        if target_row:
            target_bucket = bt_row_position_bucket(target_row) or ""

    ctx_key = (id(players), id(bt_rows) if bt_rows else None, min_games, ys, target_bucket)
    ctx = _PER_GAME_PERCENTILE_CONTEXT_CACHE.get(ctx_key)
    if ctx is None:
        cohort = [p for p in players if norm_text(p.season) == ys and p.games >= min_games]
        if bt_rows and target_bucket:
            bt_index = _get_bt_row_index(bt_rows)
            exact_rows = bt_index["exact"]
            pos_cohort = [
                p for p in cohort
                if (
                    exact_rows.get((norm_player_name(p.player), norm_team(p.team), norm_season(p.season)))
                    and bt_row_position_bucket(
                        exact_rows[(norm_player_name(p.player), norm_team(p.team), norm_season(p.season))]
                    ) == target_bucket
                )
            ]
            if pos_cohort:
        ...
        _PER_GAME_PERCENTILE_CONTEXT_CACHE[ctx_key] = ctx
    metrics = ctx["metrics"]
    return {
...
def build_player_pool_from_bt(bt_rows: list[dict[str, str]]) -> list[PlayerGameStats]:
    ...
    for r in bt_rows:
        norm_map = {norm_text(k): v for k, v in r.items()}

        def nget(*aliases: str) -> str:
            for alias in aliases:
                value = norm_map.get(norm_text(alias))
                if value is not None:
                    return str(value)
            return ""

        def nnum(*aliases: str) -> float | None:
            for alias in aliases:
                value = norm_map.get(norm_text(alias))
                num = to_float(value)
        ...
        player = nget("player_name").strip()
        team = nget("team").strip()
        season = norm_season(nget("year"))
        ...
        gp = nnum("GP", "gp") or 0.0
        if gp <= 0:
            gp = 1.0

        # Per-game box-score stats from BT.
        ppg = nnum("pts", "PTS") or 0.0
        oreb = nnum("oreb", "OREB") or 0.0
        dreb = nnum("dreb", "DREB") or 0.0
        apg = nnum("ast", "AST") or 0.0
        spg = nnum("stl", "STL") or 0.0
        bpg = nnum("blk", "BLK") or 0.0

        # Build FG/3P/FT from make/attempt pairs when available.
        two_m = nnum("twoPM", "2PM", "2PM_per_g") or 0.0
        two_a = nnum("twoPA", "2PA", "2PA_per_g") or 0.0
        three_m = nnum("TPM", "3PM") or 0.0
        three_a = nnum("TPA", "3PA") or 0.0
        ft_m = nnum("FTM") or 0.0
        ft_a = nnum("FTA") or 0.0

        fgm_pg = two_m + three_m
        fga_pg = two_a + three_a
        if fga_pg <= 0:
            fg_pct = nnum("eFG", "FG_per", "fg_per")
...
def main() -> None:
    ...
    def stage(label: str) -> None:
        ...
        print(f"[timing] {ts} | {label}: +{(now - t_last):.3f}s (total {(now - t0):.3f}s)")
        t_last = now
    ...
    stage(f"Card cache lookup {'hit' if cached_payload else 'miss'}")
    ...
    if cached_payload:
        ...
        stage("Loaded cached card sections")
        # Some cache builds intentionally skip heavy model/similarity sections.
        # Compute those lazily at render time if they are missing.
        if not player_comparisons_html:
            player_comparisons_html = build_player_comparisons_html(target, bt_rows, bio_lookup, top_n=5)
            stage("Computed missing player comparisons (cache fallback)")
        if not draft_projection_html:
            draft_projection_html = build_draft_projection_html(target, bt_rows, bio_lookup, rsci_map)
            stage("Computed missing draft projection (cache fallback)")
    else:
        bt_percentiles_html = build_bt_percentile_html(target, bt_rows, adv_rows, pbp_rows)
        stage("Built bt_percentiles_html")
        grade_boxes_html = build_grade_boxes_html(target, bt_rows)
        stage("Built grade_boxes_html")
        ...
        stage("Built self_creation_html")
        playstyles_html = build_playstyles_html(target, bt_rows)
        stage("Built playstyles_html")
        team_impact_html = build_team_impact_html(target, bt_rows)
        stage("Built team_impact_html")
        shot_diet_html = build_shot_diet_html(target, bt_rows)
        stage("Built shot_diet_html")
        player_comparisons_html = build_player_comparisons_html(target, bt_rows, bio_lookup, top_n=5)
        stage("Built player_comparisons_html")
        draft_projection_html = build_draft_projection_html(target, bt_rows, bio_lookup, rsci_map)
        stage("Built draft_projection_html")
    ...
    if not per_game_pcts:
        per_game_pcts = build_per_game_percentiles(players, target, args.min_games, bt_rows=bt_rows)
        stage("Computed per_game_percentiles fallback")
    ...
    # Keep creator tag out of panel boxes; footer is rendered in the template.
    if playstyles_html and "CREATED BY @DBCJASON" in playstyles_html:
        playstyles_html = playstyles_html.replace('<div class="play-credit">CREATED BY @DBCJASON</div>', "")
        playstyles_html = playstyles_html.replace('<div class="draft-proj-credit">CREATED BY @DBCJASON</div>', "")
    if draft_projection_html and "CREATED BY @DBCJASON" in draft_projection_html:
        draft_projection_html = draft_projection_html.replace('<div class="play-credit">CREATED BY @DBCJASON</div>', "")
    ...
    statistical_height_delta = compute_statistical_height_delta(target, bio, bt_rows, target_bt_row)
